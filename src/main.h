...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.05 * COIN;	// 5.5% annual interest
...
public:
    ...
    {
        // Moodcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Moodcoin: get transaction coin age
...
public:
    ...
    // Moodcoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Moodcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // Moodcoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Moodcoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Moodcoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Moodcoin: block index flags
    enum  
    {
